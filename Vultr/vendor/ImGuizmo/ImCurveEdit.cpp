...
static int DrawPoint(ImDrawList *draw_list, ImVec2 pos, const ImVec2 size,
                     const ImVec2 offset, bool edited)
{
    int ret = 0;
    ImGuiIO &io = ImGui::GetIO();

    static const ImVec2 localOffsets[4] = {ImVec2(1, 0), ImVec2(0, 1), ImVec2(-1, 0),
                                           ImVec2(0, -1)};
    ImVec2 offsets[4];
    for (int i = 0; i < 4; i++)
    {
         ...
         const ImRect *clippingRect, ImVector<EditPoint> *selectedPoints)
{
    ...
    const ImRect container(offset + ImVec2(0.f, ssize.y),
                           offset + ImVec2(ssize.x, 0.f));
    ...
    ImVec2 range =
        max - min + ImVec2(1.f, 0.f); // +1 because of inclusive last frame
    ...
    draw_list->AddLine(ImVec2(-1.f, -min.y / range.y) * viewSize + offset,
                       ImVec2(1.f, -min.y / range.y) * viewSize + offset, 0xFF000000,
                       1.5f);
    ...
    for (size_t cur = 0; cur < curveCount; cur++)
    {
        ...
        if ((c == highLightedCurveIndex && selection.empty() && !selectingQuad) ||
            movingCurve == c)
        ...
        for (size_t p = 0; p < ptCount - 1; p++)
        {
            ...
            if (curveType == CurveSmooth || curveType == CurveLinear)
            {
                ...
                for (size_t substep = 0; substep < subStepCount - 1; substep++)
                {
                    ...
                    const ImVec2 pos1 =
                        ImVec2(sp1.x, ImLerp(p1.y, p2.y, rt1)) * viewSize + offset;
                    const ImVec2 pos2 =
                        ImVec2(sp2.x, ImLerp(p1.y, p2.y, rt2)) * viewSize + offset;

                    if (distance(io.MousePos.x, io.MousePos.y, pos1.x, pos1.y,
                                 pos2.x, pos2.y) < 8.f &&
                        !scrollingV)
                    {
            ...
            else if (curveType == CurveDiscrete)
            {
                ...
                if ((distance(io.MousePos.x, io.MousePos.y, dp1.x, dp1.y, dp3.x,
                              dp1.y) < 8.f ||
                     distance(io.MousePos.x, io.MousePos.y, dp3.x, dp1.y, dp3.x,
                              dp3.y) < 8.f)
                    /*&& localOverCurve == -1*/)
                {
                    localOverCurve = int(c);
        ...
        {
            const int drawState =
                DrawPoint(draw_list, pointToRange(pts[p]), viewSize, offset,
    ...
    if (overSelectedPoint && io.MouseDown[0])
    {
            ...
            !selection.empty())
        {
            ...
            for (auto &sel : prevSelection)
            {
                const ImVec2 *pts = delegate.GetPoints(sel.curveIndex);
                const ImVec2 p =
                    rangeToPoint(pointToRange(originalPoints[originalIndex]) +
                                 (io.MousePos - mousePosOrigin) * sizeOfPixel);
                const int newIndex =
                    delegate.EditPoint(sel.curveIndex, sel.pointIndex, p);
    ...
    if (movingCurve != -1)
    {
        ...
        if (ptCount >= 1)
        {
            for (size_t p = 0; p < ptCount; p++)
            {
                delegate.EditPoint(
                    movingCurve, int(p),
                    rangeToPoint(pointToRange(originalPoints[p]) +
                                 (io.MousePos - mousePosOrigin) * sizeOfPixel));
            }
            ret = 1;
        }
    ...
    if (movingCurve == -1 && overCurve != -1 && ImGui::IsMouseClicked(0) &&
        selection.empty() && !selectingQuad)
    {
    ...
    if (!overCurveOrPoint && ImGui::IsMouseClicked(0) && !selectingQuad &&
        movingCurve == -1 && !overSelectedPoint && container.Contains(io.MousePos))
    {
        selectingQuad = true;
