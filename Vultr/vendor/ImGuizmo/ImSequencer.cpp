...
static bool SequencerAddDelButton(ImDrawList *draw_list, ImVec2 pos, bool add = true)
{
    ...
    draw_list->AddLine(ImVec2(delRect.Min.x + 3, midy),
                       ImVec2(delRect.Max.x - 3, midy), delColor, 2);
    if (add)
        draw_list->AddLine(ImVec2(midx, delRect.Min.y + 3),
                           ImVec2(midx, delRect.Max.y - 3), delColor, 2);
    return overDel;
}
               ...
               int *selectedEntry, int *firstFrame, int sequenceOptions)
{
    ...
    if (expanded && !*expanded)
    {
        ...
        draw_list->AddText(ImVec2(canvas_pos.x + 26, canvas_pos.y + 2), 0xFFFFFFFF,
                           tmps);
    }
    else
    {
        ...
        */
        ...
        auto drawLineContent = [&](int i, int regionHeight) {
                ...
                px >= (canvas_pos.x + legendWidth))
            {
                ...
                draw_list->AddLine(ImVec2(float(px), float(tiretStart)),
                                   ImVec2(float(px), float(tiretEnd)), 0x30606060,
                                   1);
            }
        };
        ...
        /*
                 draw_list->AddLine(canvas_pos, ImVec2(canvas_pos.x, canvas_pos.y +
           controlHeight), 0xFF000000, 1); draw_list->AddLine(ImVec2(canvas_pos.x,
           canvas_pos.y + ItemHeight), ImVec2(canvas_size.x, canvas_pos.y +
        ...
        for (int i = 0; i < sequenceCount; i++)
        {
            ...
            if (sequenceOptions & SEQUENCER_DEL)
            {
                ...
                bool overDup = SequencerAddDelButton(
                    draw_list,
                    ImVec2(contentMin.x + legendWidth - ItemHeight - ItemHeight + 2 -
                               10,
                           tpos.y + 2),
                    true);
        ...
        // clipping rect so items bars are not visible in the legend on the left when
        // scrolled
        //
        ...
        for (int i = 0; i < sequenceCount; i++)
        {
            ...
            if (!popupOpened && cy >= pos.y &&
                cy < pos.y + (ItemHeight + localCustomHeight) && movingEntry == -1 &&
        ...
        if (selected)
        {
            ...
            draw_list->AddRectFilled(
                ImVec2(contentMin.x,
                       contentMin.y + ItemHeight * *selectedEntry + customHeight),
        ...
        for (int i = 0; i < sequenceCount; i++)
        {
            ...
            ImVec2 pos =
                ImVec2(contentMin.x + legendWidth - firstFrameUsed * framePixelWidth,
                       contentMin.y + ItemHeight * i + 1 + customHeight);
            ...
            const unsigned int quadColor[] = {0xFFFFFFFF, 0xFFFFFFFF,
                                              slotColor + (selected ? 0 : 0x202020)};
            if (movingEntry == -1 &&
                (sequenceOptions &
                 SEQUENCER_EDIT_STARTEND)) // TODOFOCUS &&
            ...
            *currentFrame <= sequence->GetFrameMax())
        {
            ...
            draw_list->AddText(ImVec2(cursorOffset + 10, canvas_pos.y + 2),
                               0xFF2A2AFF, tmps);
        }
        ...
        for (auto &customDraw : customDraws)
            sequence->CustomDraw(customDraw.index, draw_list, customDraw.customRect,
                                 customDraw.legendRect, customDraw.clippingRect,
            ...
            if (sizingRBar)
            {
                ...
                else
                {
                    ...
                    int newVisibleFrameCount =
                        int((canvas_size.x - legendWidth) / framePixelWidthTarget);
            ...
            else
            {
                if (MovingScrollBar)
                {
                    ...
                    else
                    {
                        ...
                        *firstFrame = int((io.MousePos.x - panningViewSource.x) /
                                          framesPerPixelInBar) -
                                      panningViewFrame;
                        *firstFrame = ImClamp(
                            *firstFrame, sequence->GetFrameMin(),
    ...
    if (expanded)
    {
        bool overExpanded = SequencerAddDelButton(
            draw_list, ImVec2(canvas_pos.x + 2, canvas_pos.y + 2), !*expanded);
