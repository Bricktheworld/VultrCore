...
void error_callback_default(const char *fmt, ...)
{
    va_list myargs;
    va_start(myargs, fmt);
    __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, "Freetype GL Error %03x %s:\n", freetype_gl_errno, freetype_gl_message);
...
// ----------------------------------------------------- set_error_callback ---
void set_error_callback(error_callback_t error_cb) { log_error = error_cb; }
