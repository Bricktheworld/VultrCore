...
texture_atlas_t *texture_atlas_new(const size_t width, const size_t height, const size_t depth)
{
    ...
    // sampling texture
    ivec3 node = {{1, 1, static_cast<int>(width - 2)}};
...
void texture_atlas_delete(texture_atlas_t *self)
{
    ...
    texture_glyph_delete(static_cast<texture_glyph_t *>(self->special));
    if (self->data)
    {
        free(self->data);
    }
...
void texture_atlas_set_region(texture_atlas_t *self, const size_t x, const size_t y, const size_t width, const size_t height, const unsigned char *data, const size_t stride)
{
    ...
    for (i = 0; i < height; ++i)
    {
        memcpy(self->data + ((y + i) * self->width + x) * charsize * depth, data + (i * stride) * charsize, width * charsize * depth);
    }
    self->modified = 1;
}
...
ivec4 texture_atlas_get_region(texture_atlas_t *self, const size_t width, const size_t height)
{
    ...
    ivec3 *node, *prev;
    ivec4 region = {{0, 0, static_cast<int>(width), static_cast<int>(height)}};
    size_t i;
...
void texture_atlas_enlarge_texture(texture_atlas_t *self, size_t width_new, size_t height_new)
{
    ...
    self->data = static_cast<unsigned char *>(calloc(1, width_new * height_new * sizeof(char) * self->depth));
    // update atlas size
