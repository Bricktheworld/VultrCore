...
static float convert_F26Dot6_to_float(FT_F26Dot6 value) { return ((float)value) / 64.0; }
static FT_F26Dot6 convert_float_to_F26Dot6(float value) { return (FT_F26Dot6)(value * 64.0); }
...
static inline int __builtin_bswap32(uint32_t in) { return ((in >> 24) & 0xFF) | ((in >> 8) & 0xFF00) | ((in & 0xFF00) << 8) | ((in & 0xFF) << 24); }
#endif

static inline uint32_t rol(uint32_t in, uint32_t x) { return (in >> (32 - x)) | (in << x); }
...
void texture_font_default_mode(font_mode_t mode) { mode_default = mode; }
...
void texture_font_index_kerning(texture_glyph_t *self, uint32_t codepoint, float kerning)
{
    ...
    if (!*kerning_index)
    {
        *kerning_index = static_cast<float *>(calloc(0x100, sizeof(float)));
    }
...
void texture_font_generate_kerning(texture_font_t *self, FT_Library *library, FT_Face *face)
{
    ...
    GLYPHS_ITERATOR(i, glyph, self->glyphs)
    {
        ...
        GLYPHS_ITERATOR(j, prev_glyph, self->glyphs)
        {
            ...
            if (kerning.x)
            {
                texture_font_index_kerning(glyph, prev_glyph->codepoint, convert_F26Dot6_to_float(kerning.x) / HRESf);
            }
            // also insert kerning with the current added element
            FT_Get_Kerning(*face, glyph_index, prev_index, FT_KERNING_UNFITTED, &kerning);
            if (kerning.x)
            {
                texture_font_index_kerning(prev_glyph, glyph->codepoint, kerning.x / (float)(HRESf * HRESf));
            }
        }
        GLYPHS_ITERATOR_END
    }
...
int texture_font_set_size(texture_font_t *self, float size)
{
    FT_Error error   = 0;
    ...
    if (FT_HAS_FIXED_SIZES(self->face))
    {
        ...
        for (i = 0; i < self->face->num_fixed_sizes; ++i)
        {
            ...
            if (freetype_gl_warnings)
                log_error("candiate: size[%i]=%f %d*%d\n", i, new_size, self->face->available_sizes[i].width, self->face->available_sizes[i].height);
            if (ndiff < diff)
            {
                best_match = i;
                diff       = ndiff;
...
static int texture_font_init(texture_font_t *self)
{
    assert(self->atlas);
    assert(self->size > 0);
    assert((self->location == TEXTURE_FONT_FILE && self->filename) || (self->location == TEXTURE_FONT_MEMORY && self->memory.base && self->memory.size));
...
{
    texture_font_library_t *self = static_cast<texture_font_library_t *>(calloc(1, sizeof(*self)));
...
// ----------------------------------------------------- texture_font_clone ---
texture_font_t *texture_font_clone(texture_font_t *old, float pt_size)
{
    texture_font_t *self;
    FT_Error error    = 0;
    ...
    if (!self->face)
    {
        switch (self->location)
        {
            ...
            case TEXTURE_FONT_MEMORY:
                error = FT_New_Memory_Face(self->library->library, static_cast<const FT_Byte *>(self->memory.base), self->memory.size, 0, &self->face);
                if (error)
                {
                    freetype_error(error);
                    goto cleanup_library;
...
void texture_font_delete(texture_font_t *self)
{
    ...
    GLYPHS_ITERATOR(i, glyph, self->glyphs) { texture_glyph_delete(glyph); }
...
texture_glyph_t *texture_font_find_glyph_gi(texture_font_t *self, uint32_t codepoint)
{
    uint32_t i = codepoint >> 8;
    uint32_t j = codepoint & 0xFF;
    ...
    while (glyph && // if no glyph is there, we are done here
           (glyph->rendermode != self->rendermode || glyph->outline_thickness != self->outline_thickness))
    {
...
int texture_font_index_glyph(texture_font_t *self, texture_glyph_t *glyph, uint32_t codepoint)
{
    ...
    if (!*glyph_index1)
    {
        *glyph_index1 = static_cast<texture_glyph_t **>(calloc(0x100, sizeof(texture_glyph_t *)));
    }
    ...
    if ((glyph_insert = (*glyph_index1)[j]))
    {
        ...
        (*glyph_index1)[j] = glyph_insert = static_cast<texture_glyph_t *>(realloc(glyph_insert, sizeof(texture_glyph_t) * (i + 2)));
...
int texture_font_load_glyph(texture_font_t *self, const char *codepoint)
{
    ...
    return texture_font_load_glyph_gi(self, FT_Get_Char_Index(self->face, ucodepoint), ucodepoint);
}

// ------------------------------------------------ texture_font_load_glyph ---
int texture_font_load_glyph_gi(texture_font_t *self, uint32_t glyph_index, uint32_t ucodepoint)
{
    size_t i, x, y;
    ...
    unsigned char *buffer = static_cast<unsigned char *>(calloc(tgt_w * tgt_h * self->atlas->depth, sizeof(unsigned char)));
    ...
    if (!glyph_index)
    {
        if (!free_glyph)
        {
            texture_glyph_t *new_glyph = static_cast<texture_glyph_t *>(malloc(sizeof(texture_glyph_t)));
...
size_t texture_font_load_glyphs(texture_font_t *self, const char *codepoints)
{
    ...
    self->mode = static_cast<font_mode_t>((static_cast<uint8_t>(self->mode) + 1));
