...
static int DrawPoint(ImDrawList *draw_list, ImVec4 color, const ImVec2 size,
                     bool editing, ImVec2 pos)
{
    int ret = 0;
...
bool Edit(Delegate &delegate, const ImVec2 &size, int &selection)
{
    ...
    {
        int ptSel =
            DrawPoint(draw_list, pts[i], size, i == currentSelection, offset);
        if (ptSel == 2)
        {
