...
namespace Vultr
{
		...
		for (u32 i = 0; i < slab->free_blocks_len; i++)
		{
			...
			do
			{
				...
				if (slab->free_blocks[i].compare_exchange_weak(bitfield, new_val, std::memory_order_relaxed))
				{
					...
					return reinterpret_cast<byte *>(slab->heap) + ((bit_index + i) * 64);
				}
	...
	{
		ASSERT((reinterpret_cast<byte *>(data) - reinterpret_cast<byte *>(slab->heap)) % slab->block_size == 0, "Not a valid free address!");
		...
		slab->free_blocks[bitfield_index].fetch_and(~shifted, std::memory_order_relaxed);
	}
