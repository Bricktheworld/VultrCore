...
#define THREAD_SAFE_ARENAS 8
     ...
     *
     * @no_thread_safety NOTE: Use `ts_malloc` for thread safe equivalent.
     * */
    template <typename T>
    ...
    /*
     * Heap allocate a size of memory with thread saftey. This will lock a memory arena and if there are not many heaps then this can cause locking issues.
     * @param size_t size: Size in bytes of how much space the memory arena should have.
     *
     * @return T *: Pointer to the newly allocated memory.
     *
     * @thread_safe NOTE: Use `malloc` non thread safe non-locking equivalent.
     * */
    template <typename T>
